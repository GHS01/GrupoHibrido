...
async function getUserFinancialData() {
  ...
  if (useSupabase) {
    try {
      ...
      if (window.supabaseService && typeof window.supabaseService.getUserTransactions === 'function') {
        userTransactions = await window.supabaseService.getUserTransactions();
      } else if (window.getUserTransactions) {
        userTransactions = await window.getUserTransactions();
      } else {
        console.error('No method available to get transactions from Supabase');
        userTransactions = [];
      }

      // Get savings from Supabase
      if (window.supabaseService && typeof window.supabaseService.getUserSavings === 'function') {
        savingsData = await window.supabaseService.getUserSavings();
      } else if (window.getUserSavings) {
        savingsData = await window.getUserSavings();
      } else {
        console.error('No method available to get savings from Supabase');
        savingsData = { balance: 0 };
      }
    } catch (error) {
    ...
    // Use global variables populated from IndexedDB
    userTransactions = window.transactions ? window.transactions.filter(t => t.userId === userId) : [];
    savingsData = { balance: window.savingsBalance || 0 };
  }
  ...
  const savingsBalance = savingsData ? savingsData.balance : 0;
...
// Helper function to check if Supabase is being used
function isUsingSupabase() {
  // First check if the function exists in the window object
  if (typeof window.isUsingSupabase === 'function') {
    return window.isUsingSupabase();
  }

  // Then check if the supabaseIntegration module has the function
  if (window.supabaseIntegration && typeof window.supabaseIntegration.isUsingSupabase === 'function') {
    return window.supabaseIntegration.isUsingSupabase();
  }

  // Finally, fall back to checking localStorage
...
window.getFinancialInsights = getFinancialInsights;
window.generateFinancialRecommendations = generateFinancialRecommendations;