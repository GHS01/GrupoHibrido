...
window.handleChatFileAttachment = async function(event) {
  ...
  try {
    ...
    const filePath = `team_files/${window.teamChat.currentTeam}/${fileName}`;
...
window.prepareChatShareTransaction = async function() {
  try {
    if (!window.teamChat.currentUser) {
      console.error('No hay un usuario autenticado');
      return;
    ...
    newConfirmButton.addEventListener('click', async () => {
      ...
      await window.shareChatTransaction(transactionId, message);
      modal.hide();
    });
  } catch (error) {
...
window.shareChatTransaction = async function(transactionId, message) {
  try {
    if (!window.teamChat.currentUser || !window.teamChat.currentTeam) {
      console.error('No hay un usuario autenticado o no pertenece a un equipo');
      return;
...
window.prepareChatShareReport = function() {
  try {
    // Mostrar modal
    ...
    newConfirmButton.addEventListener('click', async () => {
      ...
      await window.shareChatReport(reportType, message);
      modal.hide();
    });
  } catch (error) {
...
// Compartir reporte
window.shareChatReport = async function(reportType, message) {
  try {
    if (!window.teamChat.currentUser || !window.teamChat.currentTeam) {
      console.error('No hay un usuario autenticado o no pertenece a un equipo');
      return;
